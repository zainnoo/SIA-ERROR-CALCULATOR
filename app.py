...
def calculate_sia_error(flattened_mag, flattened_axis, incision_axis, sim_sia):
    # Actual SIA: flattening at axis = negative power vector
    theta_actual = np.deg2rad(2 * flattened_axis)
    J0_actual = -flattened_mag / 2 * np.cos(theta_actual)
    J45_actual = -flattened_mag / 2 * np.sin(theta_actual)

    # Simulated SIA: always flattening at incision axis
...
st.markdown("Input the **flattened axis and magnitude** (as provided by SIA calculator). No need to enter negative signs.")
